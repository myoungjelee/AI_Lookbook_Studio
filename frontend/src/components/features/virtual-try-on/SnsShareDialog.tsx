...
export const SnsShareDialog: React.FC<{
  open: boolean;
  onClose: () => void;
  image?: string | null;
}> = ({ open, onClose, image }) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setLoading(true);
      try {
        const res: StyleTipsResponse = await virtualTryOnService.getStyleTips({
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2">
          {Object.entries(SIZES).map(([k, v]) => (
            <button
              key={k}
              onClick={() => setSize(k as SizeKey)}
              className={`px-3 py-1.5 rounded-full text-sm border ${size === k ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 border-gray-300"}`}
            >
              {v.label}
            </button>
          ))}
        ...
        <div className="text-xs text-gray-500">
          미리보기는 축소되어 보이지만, 저장 이미지는 위 해상도로 생성됩니다.
        </div>
        <div className="overflow-auto max-h-[70vh]">
          <div
            id="sns-share-capture"
            className="mx-auto shadow ring-1 ring-gray-200"
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <Button onClick={download} disabled={!image}>
            이미지 저장
          </Button>
        </div>
      </div>
