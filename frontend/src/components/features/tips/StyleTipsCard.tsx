...
export const StyleTipsCard: React.FC<StyleTipsCardProps> = ({ generatedImage }) => {
    ...
    <Card className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {typeof score === 'number' && (
            <span aria-label="AI score" title="AI score" className="text-xs px-2 py-0.5 rounded-full bg-yellow-50 text-yellow-700">
              ⭐ {score}점
            </span>
          )}
          {source && (
            <span className={`text-xs px-2 py-0.5 rounded-full ${source === 'ai' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>{source.toUpperCase()}</span>
          )}
          <Button size="sm" variant="outline" onClick={fetchTips} disabled={loading}>
            새로고침
          </Button>
        </div>
      </div>
      {loading ? (
        <div className="flex items-center gap-3 text-gray-600"><Spinner size="sm" /><span>팁을 불러오는 중…</span></div>
      ) : error ? (
        ...
        <p className="text-sm text-gray-500">생성된 이미지가 있으면 스타일 팁을 보여드립니다.</p>
      )}
    </Card>
  );
};
